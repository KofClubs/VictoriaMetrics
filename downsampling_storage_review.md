...
降采样专属代码按 block、metaindex row、reader、writer、merger、part、partition、query 八个文件划分。reader、writer、merger 的调用入口集中在内部辅助之前，对象池放在末尾。方法可见性按实际调用职责确定：`readNativeBlock`、`featureHeader` 和 merger 的 `reset` 是内部辅助；heap 和文件接口要求的大写方法须保留。
...
- 查询分辨率和特征沿原有 `Init` 参数表逐层透传，最终由 `partSearch` 选择 metaindex／index；得到原生 header 后复用原有 TSID、时间范围、`BlockRef` 及 `Block` 逻辑。不得另设平行搜索入口或在查询中聚合原始样本。
...
`openDownsamplePart` 常驻 metaindex，在 part 层独立遍历五列索引，检查列齐全、统计、共享时间戳描述，以及 timestamps／values 的连续覆盖；此过程不使用归并专属的 `downsampleReader`。打开检查不解码所有数值 payload；具体 payload 的编码、长度、行数和时间范围错误在读取时检查。
