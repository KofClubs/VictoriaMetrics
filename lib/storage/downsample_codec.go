...
const (
	downsampleFormatVersion    = 3
	downsampleSemanticsVersion = 1
	...
	downsampleFieldHeaderSize  = 91
	downsampleBlockHeaderSize  = downsampleFeaturesCount * downsampleFieldHeaderSize
	downsampleMetaindexRowSize = 96
	downsampleTimestampFeature = 255
)

// 前缀不属于 ZSTD 帧，旧程序会在读取 metaindex 时明确拒绝。
const downsampleMetaindexMagic = "VMDSMI\x00\x03"
const downsampleIndexMagic = "VMDSIX\x00\x03"
...
func (m *downsampleMetaindexRow) unmarshal(src []byte) error {
	...
	if _, err := m.TSID.Unmarshal(src[8:]); err != nil {
		return err
	}
	if _, err := m.LastTSID.Unmarshal(src[32:]); err != nil {
		return err
	}
	m.MinTimestamp = encoding.UnmarshalInt64(src[56:])
	m.MaxTimestamp = encoding.UnmarshalInt64(src[64:])
	m.BlockHeadersCount = encoding.UnmarshalUint32(src[72:])
	m.IndexBlockOffset = encoding.UnmarshalUint64(src[76:])
	m.IndexBlockSize = encoding.UnmarshalUint32(src[84:])
	m.RowsCount = encoding.UnmarshalUint64(src[88:])
