	...
	for _, tc := range []struct {
		...
		{name: "enabled_empty", enabled: true},
		{
			...
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV1(t, filepath.Join(path, "data/small/2025_01/raw"))
				writeDownsampleOpenTestManifest(t, path, "2025_01", []string{"raw"}, nil)
			},
		},
		{
			name: "historical_v1_without_manifest_or_metadata",
			setup: func(t *testing.T, path string) {
				partPath := filepath.Join(path, "data/big/2025_01/1_1_20250101000000.000_20250101000000.000_1")
				createDownsampleOpenTestV1(t, partPath)
		...
		{
			name:    "enabled_mixed_v1_v3",
			enabled: true,
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV1(t, filepath.Join(path, "data/small/2025_01/raw"))
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/big/2025_01/summary"))
				writeDownsampleOpenTestManifest(t, path, "2025_01", []string{"raw"}, []string{"summary"})
			},
		},
		{
			name: "disabled_active_v3_in_later_partition",
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV1(t, filepath.Join(path, "data/small/2024_12/raw"))
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/big/2025_01/summary"))
		...
		{
			name: "v3_only_in_snapshots",
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV3(t, filepath.Join(path, "snapshots/snapshot/data/small/2025_01/summary"))
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/small/snapshots/snapshot/2025_01/summary"))
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/big/snapshots/snapshot/2025_01/summary"))
			},
		},
		{
			name: "manifest_excludes_unpublished_v3_and_special_directories",
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV1(t, filepath.Join(path, "data/small/2025_01/raw"))
				for _, name := range []string{"unpublished", "tmp", "txn", "snapshots"} {
					createDownsampleOpenTestV3(t, filepath.Join(path, "data/small/2025_01", name))
				}
				writeDownsampleOpenTestManifest(t, path, "2025_01", []string{"raw"}, nil)
			},
		},
		...
		},
		{
			...
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/small/2025_01/summary"))
		...
		},
		{
			...
			setup: func(t *testing.T, path string) {
				createDownsampleOpenTestV3(t, filepath.Join(path, "data/small/2025_01/summary"))
				createDownsampleOpenTestV1(t, filepath.Join(path, "data/big/2025_01/raw"))
...
func TestMustOpenStorageDownsamplingRejectsV3BeforeBackgroundWork(t *testing.T) {
	setDownsampleOpenTestDedup(t, 0)
	path := t.TempDir()
	createDownsampleOpenTestV1(t, filepath.Join(path, "data/small/2024_12/raw"))
	createDownsampleOpenTestV3(t, filepath.Join(path, "data/big/2025_01/summary"))
	...
	func() {
		defer func() {
			...
			if r == nil || !strings.Contains(fmt.Sprint(r), "enable -storage.downsampling.enabled") {
				t.Fatalf("unexpected storage API result: got %v; want a v3 configuration panic", r)
			}
		}()
	...
	if after := snapshotDownsampleOpenTestFiles(t, filepath.Join(path, dataDirname)); !reflect.DeepEqual(before, after) {
		t.Fatalf("storage modified source parts or manifests before rejecting active v3")
	}
	for _, name := range []string{indexdbDirname, snapshotsDirname, cacheDirname, metadataDirname} {
		if _, err := os.Stat(filepath.Join(path, name)); !os.IsNotExist(err) {
			t.Fatalf("storage initialized %q before rejecting active v3: %v", name, err)
		}
	}
	f := vmfs.MustCreateFlockFile(path)
	vmfs.MustClose(f)
...
func TestCheckDownsamplingOpenDisabledPreservesDedup(t *testing.T) {
	...
	if after := snapshotDownsampleOpenTestFiles(t, path); !reflect.DeepEqual(before, after) {
		t.Fatalf("precheck modified v1 source files")
	}
}
...
func createDownsampleOpenTestV1(t *testing.T, path string) {
	t.Helper()
...
func createDownsampleOpenTestV3(t *testing.T, path string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatal(err)
	}
