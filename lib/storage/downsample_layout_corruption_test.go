		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.field != "" {
				...
				rewriteDownsampleCrossIndexFile(t, path, 1, func(data []byte) {
					positions := []int{58, 149, 240, 331, 422}
					if tc.field == "values" {
						positions = []int{66, 157, 248, 339, 430}
					}
...
func rewriteDownsampleCrossIndexFile(t *testing.T, path string, target int, mutate func([]byte)) {
	...
	for pos := 0; pos < len(meta); pos += 96 {
		...
		if err != nil || len(data) != 455 {
			t.Fatalf("测试 index 不是五个 91 字节单特征 header: size=%d, err=%v", len(data), err)
		}
		if pos/96 == target {
			mutate(data)
		}
		newFrame := encoding.CompressZSTDLevel(append([]byte(nil), frame[:8]...), data, 1)
		binary.BigEndian.PutUint64(mr[76:84], uint64(len(rewritten)))
		binary.BigEndian.PutUint32(mr[84:88], uint32(len(newFrame)))
			...
			for pos := 0; pos < len(meta); pos += 96 {
				...
				if !separateIndexes || pos > 0 {
					...
					for feature := 0; feature < 5; feature++ {
						copy(data[batchOffset+feature*91+34:], first)
					}
					copy(mr[56:64], first)
				}
				newFrame := encoding.CompressZSTDLevel(append([]byte(nil), frame[:8]...), data, 1)
