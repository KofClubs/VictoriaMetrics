...
func estimateDownsampleOutputSize(rows, blocks uint64) uint64 {
	...
	index := addDownsampleSpace(multiplyDownsampleSpace(downsampleBlockHeaderSize, 2), 256+uint64(len(downsampleIndexMagic)))
	metaindex := addDownsampleSpace(multiplyDownsampleSpace(downsampleMetaindexRowSize, 2), 256+uint64(len(downsampleMetaindexMagic)))
