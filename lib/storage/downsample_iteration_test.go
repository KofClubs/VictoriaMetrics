...
const (
	...
	// 第 144 个批次恰好填满首个 index，使该序列的两个批次分处相邻 index。
	downsampleIterationWideSeries = 143
...
func checkDownsampleIterationBlock(t *testing.T, br *BlockRef, tr TimeRange, expected []downsampleIterationSample, offset *int) {
	...
	if err := restored.Init(br.PartRef(), br.Marshal(nil)); err != nil {
		t.Fatal(err)
	}
	var b Block
	...
	for _, resolution := range []int64{300000, 3600000} {
		for feature := uint8(0); feature < 5; feature++ {
			...
			t.Run(fmt.Sprintf("非连续及缺失序列/%d/%d", resolution, feature), func(t *testing.T) {
				selected := []TSID{all[0], all[5], all[9], all[10], all[19], all[20], all[59], all[60], all[142], all[143], all[144], all[179]}
				beforeFirst := all[0]
				beforeFirst.MetricID -= 5
				selected = append(selected, beforeFirst)
				for _, series := range []int{5, 9, 19, 59, 142, 143, 179} {
					missing := all[series]
					missing.MetricID += 5
			...
			for _, tc := range []struct {
				...
				t.Run(fmt.Sprintf("%s/%d/%d", tc.name, resolution, feature), func(t *testing.T) {
					checkDownsampleIterationSearch(t, &ps, p, []TSID{all[142], all[143], all[144]}, q, tc.tr)
				})
			}
		}
	}
...
func TestDownsampleIterationReaderReuse(t *testing.T) {
	...
	for pass, resolution := range []int64{300000, 3600000, 300000} {
		...
		for feature := uint8(0); feature < 5; feature++ {
			wide := downsampleIterationTSID(143)
			...
			filters := []TSID{wide, missingAfterWide, last, missingAfterLast, missingBeforeFirst, first, downsampleIterationTSID(144), downsampleIterationTSID(9), downsampleIterationTSID(10), downsampleIterationTSID(19), downsampleIterationTSID(20), downsampleIterationTSID(59), downsampleIterationTSID(60)}
