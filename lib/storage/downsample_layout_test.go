...
func TestDownsampleFilePhysicalLayout(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// 96 字节 metaindex 和 91 字节单特征 header 来自格式约定，不使用生产大小常量。
			if len(meta) != tc.wantMetaRows*96 {
				t.Fatalf("metaindex 长度=%d，期望 %d 行，每行 96 字节", len(meta), tc.wantMetaRows)
			}
			...
			for pos := 0; pos < len(meta); pos += 96 {
				mr := meta[pos : pos+96]
				resolution := readDownsampleLayoutInt64(mr[0:8])
				count := binary.BigEndian.Uint32(mr[72:76])
				offset := binary.BigEndian.Uint64(mr[76:84])
				size := binary.BigEndian.Uint32(mr[84:88])
				if count == 0 || count%5 != 0 || uint64(nextBlock)+uint64(count/5) > uint64(len(blocks)) || offset != nextIndexOffset || offset+uint64(size) > uint64(len(files["index.bin"])) {
					t.Fatalf("metaindex 第 %d 行的数量或 index offset/size 错误", pos/96)
				}
				nextIndexOffset += uint64(size)
				index := decodeDownsampleLayoutFrame(t, files["index.bin"][offset:nextIndexOffset], "VMDSIX")
				if len(index) != int(count)*91 {
					t.Fatalf("index 长度=%d，与 %d 个 91 字节单特征 header 不一致", len(index), count)
				}
				group := blocks[nextBlock : nextBlock+int(count/5)]
				if readDownsampleLayoutTSID(mr[8:32]) != group[0].tsid || readDownsampleLayoutTSID(mr[32:56]) != group[len(group)-1].tsid {
				...
				for i, b := range group {
					...
					for col := 0; col < 5; col++ {
						h := index[(i*5+col)*91 : (i*5+col+1)*91]
						...
						if resolution != b.resolution || readDownsampleLayoutInt64(h[0:8]) != b.resolution || h[8] != byte(col+1) || h[9] != b.timestampPrecisionBits || readDownsampleLayoutTSID(h[10:34]) != b.tsid || binary.BigEndian.Uint32(h[82:86]) != uint32(n) {
							t.Fatalf("批次 %d 特征 %d 的标识、精度或 RowsCount 错误", nextBlock+i, col+1)
						}
						if readDownsampleLayoutInt64(h[34:42]) != b.timestamps[0] || readDownsampleLayoutInt64(h[42:50]) != b.timestamps[n-1] {
							t.Fatalf("批次 %d 特征 %d 的时间范围错误", nextBlock+i, col+1)
						}
						checkDownsampleLayoutPayload(t, h[58:66], h[74:78], timestampsOffset, timestampsPayload, files["timestamps.bin"])
						checkDownsampleLayoutPayload(t, h[66:74], h[78:82], uint64(len(expectedValues)), payload, files["values.bin"])
						u := binary.BigEndian.Uint16(h[86:88])
						if readDownsampleLayoutInt64(h[50:58]) != first || int16(u>>1)^-int16(u&1) != scale || h[88] != byte(timestampsType) || h[89] != byte(mt) || h[90] != b.precisionBits[col] {
				...
				if readDownsampleLayoutInt64(mr[56:64]) != groupMin || readDownsampleLayoutInt64(mr[64:72]) != groupMax || binary.BigEndian.Uint64(mr[88:96]) != groupRows {
			...
			wantMetadata := map[string]any{
				"FormatVersion": float64(3), "SemanticsVersion": float64(1), "Mode": "downsampling",
				"Resolutions": []any{float64(300000), float64(3600000)}, "BucketOrigin": float64(0),
				"NumericCodec": "decimal-values-v1", "Retention": "bucket-end", "MinDedupInterval": float64(0),
...
func makeDownsampleLayoutBlocks(blocksPerResolution int, singleRow bool) []*downsampleBatch {
	...
	for _, resolution := range []int64{300000, 3600000} {
		for i := 0; i < blocksPerResolution; i++ {
			b := &downsampleBatch{
				tsid:       TSID{MetricGroupID: 0x123456789abcdef0, JobID: 0x23456789, InstanceID: 0x3456789a, MetricID: uint64(i/2 + 1)},
...
func decodeDownsampleLayoutFrame(t *testing.T, data []byte, prefix string) []byte {
	t.Helper()
	if len(data) < 8 || string(data[:6]) != prefix || data[6] != 0 || data[7] != 3 {
...
func readDownsampleLayoutTSID(data []byte) TSID {
	return TSID{
		MetricGroupID: binary.BigEndian.Uint64(data[0:8]), JobID: binary.BigEndian.Uint32(data[8:12]),
		InstanceID: binary.BigEndian.Uint32(data[12:16]), MetricID: binary.BigEndian.Uint64(data[16:24]),
	}
}
