...
func QueryHandler(qt *querytracer.Tracer, startTime time.Time, w http.ResponseWriter, r *http.Request) error {
	...
	mayCache := field == nil && !httputil.GetBool(r, "nocache")
	query := r.FormValue("query")
	if len(query) == 0 {
	...
	if childQuery, windowExpr, stepExpr, offsetExpr := promql.IsRollup(query); childQuery != "" {
		...
		start = end - window
		if err := queryRangeHandler(qt, startTime, w, childQuery, start, end, step, lookbackDelta, r, ct, etfs, field); err != nil {
	...
	if mayCache && ct-start < queryOffset && start-ct < queryOffset {
...
func QueryRangeHandler(qt *querytracer.Tracer, startTime time.Time, w http.ResponseWriter, r *http.Request) error {
	...
	if err := queryRangeHandler(qt, startTime, w, query, start, end, step, lookbackDelta, r, ct, etfs, field); err != nil {
